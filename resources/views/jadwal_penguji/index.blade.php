                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse ($jadwalPengujis as $jadwal)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $jadwal->deskripsi ?? '-' }}</td>
                                @if (Auth::user()->isAdmin())
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('jadwal-penguji.edit', $jadwal->id) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-medium transition-colors duration-150 border border-blue-200">
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500" id="deleteModalMessage">
                        Apakah Anda yakin ingin menghapus <span id="deleteItemName" class="font-semibold"></span>? Tindakan ini tidak dapat dibatalkan.
                    </p>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        confirmDeleteBtn.addEventListener('click', function() {
            if (jadwalToDeleteId) {
                deleteForm.action = `/jadwal-penguji/${jadwalToDeleteId}`;
                deleteForm.submit();
            }
            hideDeleteModal();
        });
